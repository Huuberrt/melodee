...
public class DirectoryProcessorToStagingServiceTests : ServiceTestBase
{
    ...
    [Fact]
    public async Task ProcessDirectoryAsync_WithManySubdirectories_ShouldCompleteEfficiently()
    {
        // Arrange
        var processor = CreateDirectoryProcessorService();
        await processor.InitializeAsync(TestsBase.NewPluginsConfiguration());
        
        // Create test structure with many subdirectories
        var baseTestPath = _mockFileSystemService.CombinePath(_testDataDirectory, "ManyDirs");
        _mockFileSystemService.SetDirectoryExists(baseTestPath);
        
        // Create 20 subdirectories to test parallel processing
        for (int i = 1; i <= 20; i++)
        {
            var subDirPath = _mockFileSystemService.CombinePath(baseTestPath, $"Album{i:D2}");
            SetupTestDirectoryWithAlbumJson(subDirPath);
        }
        ...
        Assert.NotNull(result);
        Assert.NotNull(result.Data);
        Assert.True(stopwatch.ElapsedMilliseconds < 30000, $"Processing took too long: {stopwatch.ElapsedMilliseconds}ms");
        
        // Verify that processing completed without major errors
        var hasSignificantErrors = result.Errors?.Any(e => !e.Message.Contains("Invalid") && !e.Message.Contains("not found")) ?? false;
        Assert.False(hasSignificantErrors, "Should not have significant processing errors");
    }

    [Fact]
    public async Task ProcessDirectoryAsync_WithConcurrentAccess_ShouldHandleParallelism()
    {
        // Arrange
        var processor = CreateDirectoryProcessorService();
        await processor.InitializeAsync(TestsBase.NewPluginsConfiguration());
        ...
        var testDirectories = new List<FileSystemDirectoryInfo>();
        for (int i = 1; i <= 5; i++)
        {
            var dirPath = _mockFileSystemService.CombinePath(_testDataDirectory, $"Concurrent{i}");
            SetupTestDirectoryWithAlbumJson(dirPath);
            testDirectories.Add(new FileSystemDirectoryInfo
            {
                Path = dirPath,
                Name = $"Concurrent{i}"
            });
        ...
        // Act - Process multiple directories concurrently
        var tasks = testDirectories.Select(dir => 
            processor.ProcessDirectoryAsync(dir, null, 1));
        ...
        // Verify no major exceptions occurred during concurrent processing
        var exceptions = results.SelectMany(r => r.Errors ?? [])
            .Where(e => !e.Message.Contains("Invalid") && !e.Message.Contains("not found"))
            .ToList();
        
        Assert.True(exceptions.Count <= 1, $"Too many exceptions during concurrent processing: {exceptions.Count}");
    }

    [Fact]
    public async Task ProcessDirectoryAsync_MemoryUsage_ShouldNotExceedReasonableLimits()
    {
        // Arrange
        var processor = CreateDirectoryProcessorService();
        await processor.InitializeAsync(TestsBase.NewPluginsConfiguration());
        
        var testDirPath = _mockFileSystemService.CombinePath(_testDataDirectory, "MemoryTest");
        SetupTestDirectoryWithAlbumJson(testDirPath);
        ...
        // Assert
        Assert.NotNull(result);
        Assert.True(memoryIncreaseMB < 100, $"Memory usage increased by {memoryIncreaseMB:F2}MB, which seems excessive for a single directory");
    }
